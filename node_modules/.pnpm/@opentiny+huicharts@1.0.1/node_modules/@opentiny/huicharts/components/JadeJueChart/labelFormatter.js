/**
 * Copyright (c) 2024 - present OpenTiny HUICharts Authors.
 * Copyright (c) 2024 - present Huawei Cloud Computing Technologies Co., Ltd.
 *
 * Use of this source code is governed by an MIT-style license.
 *
 * THE OPEN SOURCE SOFTWARE IN THIS PRODUCT IS DISTRIBUTED IN THE HOPE THAT IT WILL BE USEFUL,
 * BUT WITHOUT ANY WARRANTY, WITHOUT EVEN THE IMPLIED WARRANTY OF MERCHANTABILITY OR FITNESS FOR
 * A PARTICULAR PURPOSE. SEE THE APPLICABLE LICENSES FOR MORE DETAILS.
 *
 */
// 处理第二种数据且有标定值的sum和max
var setChildMax = function setChildMax(data, max) {
  data.forEach(function (dataItem) {
    if (dataItem.children) {
      dataItem.children.forEach(function (child) {
        child.max = max;
      });
    } else {
      dataItem.max = max;
    }
  });
};

// 处理第一种数据且无标定值的sum和max
var setNoChildDefaultMax = function setNoChildDefaultMax(data, angleAxis, fill) {
  data.forEach(function (dataItem) {
    // data中的value是数值类型，处理数据
    if (!dataItem.children) {
      var sum = 0;
      data.forEach(function (item) {
        sum += item.value;
      });
      // 当数据全为0时，手动设置sum和max使其不为0，避免视图丢失
      if (sum === 0) {
        sum = 1;
      }
      angleAxis.max = sum * 4 / 3;
      angleAxis.sum = sum;
      if (fill) {
        angleAxis.max = sum;
      }
    }
  });
};

// 处理第二种数据且无标定值的sum和max
var setChildDefaultMax = function setChildDefaultMax(data, angleAxis, fill) {
  data.forEach(function (dataItem) {
    if (dataItem.children) {
      var sum = 0;
      data.forEach(function (item) {
        item.children.forEach(function (item_) {
          sum += item_.value;
        });
      });
      // 当数据全为0时，手动设置sum和max使其不为0，避免视图丢失
      if (sum === 0) {
        sum = 1;
      }
      angleAxis.sum = sum;
      angleAxis.max = sum * 4 / 3;
      if (fill) {
        angleAxis.max = sum;
      }
    }
  });
};

// 配置玉闋图外围坐标系百分比展示文本
var setAxisLabel = function setAxisLabel(sum, iChartOption, baseOpt) {
  if (iChartOption.max) {
    baseOpt.angleAxis.sum = iChartOption.max;
    var formatter = function formatter(params) {
      if (params === iChartOption.max) {
        return '100%';
      }
      if (params / iChartOption.max * 100 > 100) {
        return '';
      }
      var x = Math.ceil(params / iChartOption.max * 100);
      return Math.ceil(x / 10) * 10 + "%";
    };
    return formatter;
  } else {
    var _formatter = function _formatter(params) {
      if (params === sum) {
        return '100%';
      }
      if (params / sum * 100 > 100) {
        return '';
      }
      var x = Math.ceil(params / sum * 100);
      return Math.ceil(x / 10) * 10 + "%";
    };
    return _formatter;
  }
};

/**
 * 此函数两部分作用：（1）根据data和max,将数据分为四种数据类型，分类处理angleAxis的sum和max值
 * （2）根据用户传入的formatter，配置极坐标系文本
 * @param {*} iChartOption
 * @param {*} baseOpt
 */
var handleLabelFormatter = function handleLabelFormatter(iChartOption, baseOpt) {
  var max = iChartOption.max,
    data = iChartOption.data,
    _iChartOption$fill = iChartOption.fill,
    fill = _iChartOption$fill === void 0 ? false : _iChartOption$fill,
    _iChartOption$type = iChartOption.type,
    type = _iChartOption$type === void 0 ? 'base' : _iChartOption$type;
  var angleAxis = baseOpt.angleAxis;
  if (type === 'process') {
    fill = true;
  }
  // 有标定值
  if (max) {
    // 第一种数据类型
    angleAxis.sum = max;
    angleAxis.max = max * 4 / 3;
    // 第二种数据类型
    setChildMax(data, max);
    if (fill) {
      angleAxis.max = max;
    }
  }
  // 无标定值
  else {
    if (type === 'process') {
      angleAxis.max = 100;
      angleAxis.sum = 100;
    } else if (type === 'base') {
      // 第一种数据类型
      setNoChildDefaultMax(data, angleAxis, fill);
    } else {
      // 第二种数据类型
      setChildDefaultMax(data, angleAxis, fill);
    }
  }

  /**
   * 处理显示文本:根据用户传入的formatter，配置极坐标系文本
   * 传入"percent"或者未传，展示百分比
   * 传入"number",展示纯数值
   * 传入函数，用户自定义展示内容
   */
  var formatter = angleAxis.axisLabel.formatter;
  switch (formatter) {
    case 'percent':
      formatter = setAxisLabel(angleAxis.sum, iChartOption, baseOpt);
      break;
    case 'number':
      formatter = undefined;
      break;
    case undefined:
      formatter = setAxisLabel(angleAxis.sum, iChartOption, baseOpt);
      break;
  }
  if (type === 'process') {
    angleAxis.axisLabel.show = false;
    return;
  }
  angleAxis.axisLabel.formatter = formatter;
  angleAxis.axisTick.show = true; // 展示刻度线
};
export { handleLabelFormatter };

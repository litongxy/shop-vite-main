...
function markLineArea(baseOption, iChartOption, YAxiMin) {
  if (iChartOption.area && iChartOption.markLine && iChartOption.markLine.top && isNumber(iChartOption.markLine.top) && iChartOption.markLine.topChangeColor !== false) {
    ...
    var topColor = codeToRGB(markLine.topColor, 0.15) || codeToRGB(Theme.config.colorState.colorError, 0.15);
    baseOption.series.forEach(function (item, index) {
      var maxValue = max(item.data);
      var color = getColor(colors, index);
      var colorTo = codeToRGB(color, 0);
      var colorFrom = codeToRGB(color, 0.15);
...
function splitArea(baseOption, iChartOption, YAxiMin) {
  if (iChartOption.area && iChartOption.splitLine) {
    ...
    baseOption.series.forEach(function (item, index) {
      var maxValue = max(item.data);
      var color = getColor(colors, index);
      var colorTo = codeToRGB(color, 0.15);
      var colorFrom = codeToRGB(color, 0.15);
...
function topArea(baseOption, iChartOption, YAxiMin) {
  // 添加默认areaStyle
  defaultArea(baseOption, iChartOption);
